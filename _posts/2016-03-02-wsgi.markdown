...
<h2>4.规范概述</h2>
WSGI接口有两侧：server（或gateway）侧，application（或framework）侧。server侧调用application侧提供的callable的对象。该对象该如何提供应由server（或gateway）来决定。这是基于这样一个假设：某些servers（或gateways）需要application的部署人员写一个短的脚本来创建server（或gateway）的实例，并且为它提供一个application对象。
