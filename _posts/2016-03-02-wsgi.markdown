...
<h2>2.摘要</h2>
本文定义了一种web服务器和python web应用程序（或框架）之间的建议标准接口，来增强web应用程序与多种web servers之间的可移植性。
<h2>3.基本原理和目标</h2>
python当前已有很多web应用程序框架，如Zope、Quixote、Webware、SkunkWeb、PSO和Twisted Web（[1]）。如此之多的选择对于python的新手们来说是个问题，通常他们所选的web框架会限制他们对web servers的选择，反之亦然。

相比之下，虽然Java也拥有很多可选的web应用程序框架，但其“servlet” API使任何使用Java web应用程序框架所写的应用程序可以在任意支持servlet API的web servers里运行成为可能。

Python web servers这种API的可用性及广泛性——无论这些servers 是用Pyton写的（如[Medusa]），嵌入Python（如[mod_python]），或通过网关协议调用的Python（如CGI、FastCGI等）——将使对web server和框架的选择分开，让用户自由选择一对他们想要的，同时让框架和server开发者专注于他们所擅长的领域。

因此，本PEP建议了一种简单且通用的web servers和web应用程序（或框架）直接的接口：即Python Web Server Gateway Interface（WSGI）。

但是一个WSGI细则的存在并没有解决Python web应用程序的servers和框架目前的状况。为了达到效果，server和框架的作者和维护人员需要自己实现WSGI。

然而没有已存在的servers或框架支持，实现WSGI几乎没有捷径。因此，WSGI必须容易实现，让人们的以合理较低的代价来投入。

所以，接口在server和框架侧的简单实现，对WSGI接口工具是决定性的，且是任何设计决策是主要的标准。

实现的简单性，对于一个框架作者和web应用程序作者来说是不一样的。WSGI提出一个绝对“不提供不必要东西”的接口给框架作者，因为其他诸如response对象和cookie操作只会阻碍已有框架对这些的处理。此外，WSGI的目的是促进已有servers和应用程序（或框架）直接的简单互联，而不是创建一个新的web框架。
