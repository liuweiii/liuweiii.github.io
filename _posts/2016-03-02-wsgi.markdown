...
<h3>4.1 Application/Framework侧</h3>
...
def run_with_cgi(application):
    ...
    try:
        ...
        if hasattr(result, 'close'):
            result.close()
{% endhighlight %}
<h3>3. Middleware:工作在两侧的组件</h3>
既然一个单独的对象既可以对某些application(s)扮演server，也可以扮演对某些server(s)扮演application。“middleware”就可以提供这样的函数：
<ul>
<li>基于目标URL，路由一个request给不同的application对象，相应地重写environ。</li>
<li>允许多个applications（或frameworks）并行地运行在相同的进程里。</li>
<li>通过网络转发请求和响应实现负载均衡和远程处理。</li>
<li>执行内容的后期处理，如应用XSL样式表。</li>
</ul>
